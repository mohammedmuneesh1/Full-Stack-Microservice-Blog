...
import { BLOG_AI_PARAGRAPH_CORRECTOR_API, BLOG_AI_TITLE_CORRECTOR_API, CREATE_BLOG_API, EDIT_BLOG_API } from "@/api/blogApi";
import toast from "react-hot-toast";
import { BLOG_AI_CONTENT_GRAMMER_CORRECTOR_API } from "@/api/blogApi"; 
import { useAppcontext } from "@/context/AppContext";
import { Blog } from "../BlogByIdClientPage";
...
const EditClientPage:React.FC<EditClientPageInterface> = ({blogData}) => {
...
useEffect(() => {
  if (!blogData?.success) {
    toast.success(blogData?.response);
    return;
  }

if (blogData && user && blogData?.data?.author?._id !== user?._id) {
  toast.error("Only Blog creator can do the updation.");
  return router.back();
}
  ...
  const [preview, setPreview] = useState(blogData?.data?.image?.url ?? "");
  ...
  const submitHandler = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    const res = await EDIT_BLOG_API(String(blogData?.data?.id),payload);
    setLoading(false);
    if (res?.success) {
